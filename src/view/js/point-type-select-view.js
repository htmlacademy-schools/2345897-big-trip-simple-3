...
export default class PointTypeSelectView extends ViewRadioButton {
  constructor() {
    super(...arguments);
   ...
   */
  onKeyDown(event) {
    if (UserCommand.EXIT.includes(event.key) && this.toggleView.checked) {
    ...
    if (UserCommand.NEXT.includes(event.key)) {
      event.preventDefault();
    ...
    if (UserCommand.PREVIOUS.includes(event.key)) {
      event.preventDefault();
    ...
    if (UserCommand.CONFIRM.includes(event.key)) {
      event.preventDefault();
